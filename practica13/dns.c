...
void main()
{
  ...
  if (strcmp(direccion, direc) == 0)
  {
    ...
    /* Configuramos el buffer a enviar, colocamos el encabezado, entradas de solicitud, etc. recibimos la longitd final de la solicitud */
...
int configurar_buffer(unsigned char encabezado[12], unsigned char direccion[50], unsigned char buffer[512])
{
  ...
  /* Si queremos que la función siga dividiendo nuestra cadena; en las siguientes llamadas a la misma no debemos pasarle la cadena (sino NULL), y también los delimitadores. */
  if (token != NULL)
  {
    while (token != NULL)
    {
...
int recorrer_trama(unsigned char buffer[512], int apuntador)
{
  ...
  /* Incrementemos en +2 para imprimir la clase internet + 1 para quedar en el primer byte del tiempo de vida */
  imprimir_clase();
  apuntador += 3;
  imprimir_tiempo(buffer, apuntador);
  /* Incrementamos en +4 para quedar en el primer byte del tam de los datos + 1 para quedar en el segundo byte */
  apuntador += 5;
