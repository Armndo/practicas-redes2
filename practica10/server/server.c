...
int udp_socket, lbind, tamRecibido, lrecv, tamEnviado, tamParcial, tamTotal;
struct sockaddr_in servidor, cliente;
unsigned char data[516], estrACK[4], mensajeRecibido[516], estructData[516];
FILE *fw,
    *fr;
...
int main()
{
  ...
  else
  {
    ...
    else
    {
      ...
      while (1)
      {
        tamRecibido = recvfrom(udp_socket, mensajeRecibido, sizeof(mensajeRecibido), 0, (struct sockaddr *)&cliente, &lrecv);
        if (tamRecibido == -1)
        {
        }
        else
        {
          if (mensajeRecibido[1] == 0x01)
          {
            printf("Peticion de lectura\n");
            printf("nombre arch: %s\n", mensajeRecibido + 2);
            int tamError, estr;
            fr = fopen(mensajeRecibido + 2, "rb");
            if (fr == NULL)
            {
          ...
          if (mensajeRecibido[1] == 0x02)
          {
            printf("Peticion de Escritura\n");
            printf("nombre arch: %s\n", mensajeRecibido + 2);
            int tamError, estr;
            fw = fopen(mensajeRecibido + 2, "wb");
            if (fw == NULL)
            {
            ...
            else
            {
              ...
              while (1)
              {
                tamRecibido = recvfrom(udp_socket, mensajeRecibido, sizeof(mensajeRecibido), MSG_DONTWAIT, (struct sockaddr *)&cliente, &lrecv);
                if (tamRecibido == -1)
                {
                }
                else
...
int EsperarACK(unsigned char *paq, int tam)
{
  ...
  {
    tamRecibido = recvfrom(udp_socket, mensajeRecibido,
                           sizeof(mensajeRecibido), MSG_DONTWAIT, (struct sockaddr *)&cliente, &lrecv);
    if (tamRecibido == -1)
    {
    }
    if (tamRecibido == -1)
    ...
    {
      if (mensajeRecibido[1] == 0x04 && mensajeRecibido[2] == numPaqHex[0] &&
          mensajeRecibido[3] == numPaqHex[1])
      {
        return 4;
      }
      if (mensajeRecibido[1] == 0x05)
      {
        return 5;
      }
    }
...
void recivirMensaje()
{
  ...
  while (1)
  {
    tamRecibido = recvfrom(udp_socket, data, 512, MSG_DONTWAIT, (struct sockaddr *)&cliente, &lrecv);
    if (tamRecibido == -1)
    {
    }
    if ((tamRecibido != -1 && tamRecibido < 512) || tamRecibido == 0)
    {
...
{
  unsigned char codOP[2] = {0x00, 0x04};
  numPaqHex[0] = mensajeRecibido[2];
  numPaqHex[1] = mensajeRecibido[3];
  memcpy(paq + 0, codOP, 2);
  memcpy(paq + 2, numPaqHex, 2);
  return 4;
}