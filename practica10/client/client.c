...
int udp_socket, dbind, tamEnviado, tamTotal = 0, tamParcial, lrecv, tamRecibido, tamData;
unsigned char data[516], estructData[516], estrPeticionLectura[516], estrPeticionEscritura[516], estrACK[4], mensajeRecibido[516], nomArch[30];
            ...
            else
            {
              ...
              { // Limitar a 10 MB
                ...
                else
                {
                  ...
                  if (tamRecibido >= 516)
                  {
                    if (mensajeRecibido[2] == estrACK[2] && mensajeRecibido[3] == estrACK[3])
                    {
                    }
                    else
                    {
                  ...
                  if (tamRecibido < 516 || tamTotal > 10 * 1024 * 1024)
                  { // Parar si se alcanza el límite de 10 MB
                    fwrite(mensajeRecibido + 4, 1, tamRecibido - 4, fw);
                    printf("Peso del Archivo: %i bytes\n", tamTotal);
                    EnviarACK();
                    break;
